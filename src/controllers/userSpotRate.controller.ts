...
export const getUserCommodity = async (req: Request, res: Response, next: NextFunction): Promise<any> => {
  try {
...
export const updateUserSpread = async (req: Request, res: Response): Promise<any> => {
  try {
    ...
    let userSpotRate = await UserSpotRateModel.findOne({ createdBy: new mongoose.Types.ObjectId(adminId) });
...
export const deleteUserCommodity = async (req: Request, res: Response, next: NextFunction): Promise<any> => {
  try {
    const adminId = req.params.adminId as string;
    const categoryId = req.params.categoryId as string;
