...
export const getTemplateConfig = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    res.status(500).json({ success: false, message: 'Failed to load template config' });
  }
};

export const upsertTemplateConfig = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const userId = req.user?.id;
    const { templateId } = req.params;
    ...
    res.status(500).json({ success: false, message: 'Failed to save template config' });
  }
};
