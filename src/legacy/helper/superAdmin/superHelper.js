...
export const fetchAdminData = async () => {
  try {
    ...
    return admins.map((admin) => {
      ...
      try {
        decryptedPassword = decryptPassword(admin.password, admin.passwordAccessKey);
      } catch (err) {
        console.error(`Decryption failed for admin ${admin._id}:`, err.message);
        decryptedPassword = admin.password; // fallback for old data
      }
      return {
        ...admin.toObject(),
