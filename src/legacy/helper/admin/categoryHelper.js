...
const createMainCategoryHelper = async (categoryData) => {
  try {
    ...
    // Check if a category with same name AND same creator exists
    const existingCategory = await MainCategory.findOne({ 
      name, 
      createdBy
    });
    if (existingCategory) {
      throw createAppError("Main Category with this name already exists for this creator", 400);
    }

    // If no matching category with same name and creator, create new category
