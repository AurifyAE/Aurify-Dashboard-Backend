import {
  addLondonFix,
  getAllLondonFix,
  getLondonFixById,
  updateLondonFix,
  deleteLondonFix,
} from "../../helper/superAdmin/londonFixHelper.js";
import { io } from "../../config/socket_io.js";

// ✅ Add London Fix
export const addLondonFixController = async (req, res) => {
  try {
    const { success, londonFix, message } = await addLondonFix(req.body);

    if (!success) {
      return res.status(400).json({ success: false, message });
    }

    // 🔥 Emit real-time update event to all connected clients
    io.emit("londonfix-updated", {
      message: "London Fix added or updated",
      londonFix,
    });

    return res.status(200).json({
      success: true,
      londonFix,
      message: "London Fix added/updated successfully",
    });
  } catch (error) {
    return res.status(500).json({
      success: false,
      message: "Internal Server Error",
      error: error.message,
    });
  }
};

// ✅ Get All London Fix
export const getAllLondonFixController = async (req, res) => {
  const response = await getAllLondonFix();
  return res.status(response.success ? 200 : 400).json(response);
};

// ✅ Get by ID
export const getLondonFixByIdController = async (req, res) => {
  const response = await getLondonFixById(req.params.id);
  return res.status(response.success ? 200 : 404).json(response);
};

// ✅ Update London Fix
export const updateLondonFixController = async (req, res) => {
  try {
    const { success, message, londonFix } = await updateLondonFix(
      req.params.id,
      req.body
    );

    if (!success) {
      return res.status(message.includes("not found") ? 404 : 400).json({
        success: false,
        message,
      });
    }

    // 🔥 Emit real-time update event
    io.emit("londonfix-updated", {
      message: "London Fix updated",
      londonFix,
    });

    return res.status(200).json({
      success: true,
      londonFix,
      message: "London Fix updated successfully",
    });
  } catch (error) {
    return res.status(500).json({
      success: false,
      message: "Internal Server Error",
      error: error.message,
    });
  }
};

// ✅ Delete London Fix
export const deleteLondonFixController = async (req, res) => {
  const response = await deleteLondonFix(req.params.id);

  if (response.success) {
    // 🔥 Emit deletion event
    io.emit("londonfix-updated", {
      message: "London Fix deleted",
      deletedId: req.params.id,
    });
  }

  return res.status(response.success ? 200 : 404).json(response);
};