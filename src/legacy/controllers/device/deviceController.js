...
import { getRetailGoldRatesByAdminId } from "../../helper/retailGoldRateHelper.js";
import {
  getAllLondonFix,
...
export const getRetailGoldRates = async (req, res, next) => {
  try {
    const { adminId } = req.params;
    const { success, rates, message } = await getRetailGoldRatesByAdminId(adminId);
    ...
    return res.status(200).json({
      success: true,
      rates,
      message: "Fetching retail gold rates successfully",
    });
  } catch (error) {
    next(error);
  }
