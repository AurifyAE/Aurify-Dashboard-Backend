...
async function handleNotificationEvent(event, payload) {
    try {
        ...
        const templateFn = notification_templates_1.NotificationTemplates[event];
        if (!templateFn) {
...
Object.values(eventBus_1.NotificationEvents).forEach((event) => {
    eventBus_1.NotificationEventBus.subscribe(event, (payload) => {
