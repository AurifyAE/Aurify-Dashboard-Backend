...
const updateProfile = async (req, res) => {
  try {
    ...
    if (req.body.slug !== undefined && req.body.slug !== merchant.slug) {
      ...
      const screens = await PublishedScreen_1.ScreenRecord.find({
        merchantId: merchant.merchantId,
      });
    ...
    (0, eventBus_1.emitBusinessEvent)(eventBus_1.NotificationEvents.PROFILE_UPDATED, {
      merchantId: merchant.merchantId,
      actorName: actor.name,
      actor,
    });
...
const deleteLayout = async (req, res) => {
  try {
    ...
    const layout = await ScreenLayout_1.default
      .findOne({ layoutId, merchantId: merchant.merchantId })
      .lean();
    ...
    await PublishedScreen_1.PublishedLayoutVersion.deleteMany({
      layoutId,
      merchantId: merchant.merchantId,
    });
    await PublishedScreen_1.ScreenRecord.updateMany(
      { layoutId, merchantId: merchant.merchantId },
    ...
    if (layout) {
      ...
      (0, eventBus_1.emitBusinessEvent)(eventBus_1.NotificationEvents.LAYOUT_UNPUBLISHED, {
        merchantId: merchant.merchantId,
        actorName: actor.name,
...
const publishLayout = async (req, res) => {
  try {
    ...
    (0, eventBus_1.emitBusinessEvent)(eventBus_1.NotificationEvents.LAYOUT_PUBLISHED, {
      merchantId: merchant.merchantId,
      actorName: actor.name,
      layoutName: layout.name,
      actor,
...
const listMerchantCommodities = async (req, res) => {
  try {
    ...
    const doc = await MerchantCommodity_1.default
      .findOne({ merchantId: merchant.merchantId })
      .lean();
...
const deleteMerchantCommodity = async (req, res) => {
  try {
    ...
    const id = req.params.id;
    const doc = await MerchantCommodity_1.default.findOneAndUpdate(
      { merchantId: merchant.merchantId },
...
const deleteNews = async (req, res) => {
  try {
    ...
    const result = await MerchantNews_1.default.findOneAndUpdate(
      { merchantId: merchant.merchantId },
      { $pull: { news: { _id: new mongoose_1.default.Types.ObjectId(req.params.id) } } },
      { new: true }
    );
    if (!result) {
      res.status(404).json({ success: false, message: 'News item not found' });
...
const getLiveScreen = async (req, res) => {
  try {
    ...
    const newsItems = news?.news
      ? news.news.filter((n) => n.active).sort((a, b) => (b.priority ?? 1) - (a.priority ?? 1))
      : [];
    res.status(200).json({
      success: true,
      data: {
