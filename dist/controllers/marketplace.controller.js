...
exports.getLiveScreen = exports.upsertNews = exports.listNews = exports.upsertMerchantCommodity = exports.listMerchantCommodities = exports.publishLayout = exports.saveLayout = exports.listLayouts = exports.listMerchantThemes = exports.installTheme = exports.listThemes = exports.getProfile = exports.updateProfile = exports.approveMerchant = exports.registerMerchant = exports.getMyMerchant = void 0;
...
const listThemes = async (_req, res) => {
    try {
        ...
        const themes = await MarketplaceTheme_1.default.find({ active: true }).sort({ category: 1, name: 1 }).lean();
...
const listMerchantThemes = async (req, res) => {
    try {
        ...
        const themes = await MerchantTheme_1.default.find({ merchantId: merchant.merchantId }).sort({ installedAt: -1 }).lean();
        res.status(200).json({ success: true, data: themes });
    }
    catch (err) {
...
const listLayouts = async (req, res) => {
    try {
        ...
        const layouts = await ScreenLayout_1.default.find({ merchantId: merchant.merchantId }).sort({ updatedAt: -1 }).lean();
...
const publishLayout = async (req, res) => {
    try {
        ...
        const layout = await ScreenLayout_1.default.findOne({ layoutId: req.params.layoutId, merchantId: merchant.merchantId }).lean();
        if (!layout) {
        ...
        const installedTheme = await MerchantTheme_1.default.findOne({ merchantId: merchant.merchantId, themeId: layout.themeId }).lean();
        if (!installedTheme) {
...
const listMerchantCommodities = async (req, res) => {
    try {
        ...
        const commodities = await MerchantCommodity_1.default.find({ merchantId: merchant.merchantId }).sort({ createdAt: -1 }).lean();
...
const upsertMerchantCommodity = async (req, res) => {
    try {
        ...
        const commodity = id
            ? await MerchantCommodity_1.default.findOneAndUpdate({ _id: id, merchantId: merchant.merchantId }, { $set: payload }, { new: true, runValidators: true })
            : await MerchantCommodity_1.default.create({ ...payload, merchantId: merchant.merchantId });
        res.status(id && !commodity ? 404 : 200).json(commodity
...
const listNews = async (req, res) => {
    try {
        ...
        const news = await MerchantNews_1.default.find({ merchantId: merchant.merchantId }).sort({ priority: -1, createdAt: -1 }).lean();
...
const upsertNews = async (req, res) => {
    try {
        ...
        const id = req.params.id;
        const news = id
            ? await MerchantNews_1.default.findOneAndUpdate({ _id: id, merchantId: merchant.merchantId }, { $set: payload }, { new: true, runValidators: true })
...
const getLiveScreen = async (req, res) => {
    try {
        const merchant = await Merchant_1.default.findOne({ slug: req.params.merchantSlug, status: "Active" }).lean();
        if (!merchant) {
        ...
        const screen = await PublishedScreen_1.ScreenRecord.findOne({ merchantId: merchant.merchantId, screenSlug, live: true }).lean();
        if (!screen) {
        ...
        const [layout, theme, profile, commodities, news, spotRateSettings] = await Promise.all([
            ...
            MerchantCommodity_1.default.find({ merchantId: merchant.merchantId, active: true }).sort({ createdAt: -1 }).lean(),
            MerchantNews_1.default.find({ merchantId: merchant.merchantId, active: true }).sort({ priority: -1 }).lean(),
