...
exports.deleteCommodity = exports.updateCommodity = exports.createCommodity = exports.listCommodities = void 0;
...
const listCommodities = async (req, res) => {
    try {
        const userId = getUserId(req);
        const commodities = await Commodity_1.default.find({ adminId: userId })
            .sort({ createdAt: -1 })
            .lean();
        res.status(200).json({
            success: true,
...
exports.listCommodities = listCommodities;
...
const createCommodity = async (req, res) => {
    try {
        ...
        const existing = await Commodity_1.default.findOne({
            adminId: userId,
        ...
        const doc = await Commodity_1.default.create({
            adminId: userId,
            metal: normalizedMetal,
            purity: normalizedPurity,
            unit: normalizedUnit,
...
const updateCommodity = async (req, res) => {
    try {
        ...
        const doc = await Commodity_1.default.findOneAndUpdate({ _id: id, adminId: userId }, { $set: update }, { new: true, runValidators: true });
        if (!doc) {
            res.status(404).json({
                success: false,
...
const deleteCommodity = async (req, res) => {
    try {
        const userId = getUserId(req);
        const { id } = req.params;
        const doc = await Commodity_1.default.findOneAndDelete({ _id: id, adminId: userId });
        if (!doc) {
            res.status(404).json({
                success: false,
                message: "Commodity not found",
